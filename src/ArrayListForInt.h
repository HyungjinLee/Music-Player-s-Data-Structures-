...
public:
	...
	*	@param	data	찾고자 하는 id가 입력된 int값. 발견한 item이 리턴된다.
	...
	*	@param	data	제거하고자 하는 id가 입력된 int값.
	...
	*	@param	data	교체하고자 하는 id와 레코드를 가진 int값.
	*	@return	성공시 1, 실패시 0을 리턴한다.
	*/
	...
	*	@param	data	찾고자 하는 id가 입력된 int값.
	*	@return	성공(발견)시 1, 실패시 0을 리턴한다.
	*/
	int GetBinarySearch(int& data);
private:
	int m_Array[100];  ///< list array.
