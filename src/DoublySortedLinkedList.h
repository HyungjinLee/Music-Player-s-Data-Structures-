...
struct DoublyNodeType
{
	...
	DoublyNodeType *next; // 노드의 다음을 가리키는 포인터
};
...
{
	friend class DoublyIterator<T>; //DoublyIterator의 친구 클래스
public:
	/**
	*	디폴트 생성자.
	*/
	...
	*	@brief	리스트를 비운다.
	*	@pre	없음.
	*	@post	m_header와 m_trailer를 제외한 모든 데이터를 지운다.
	*/
	void MakeEmpty();
	...
	*/
	...
	/**
	*	@brief	입력받은 아이템의 정보와 비교하여 같은 리스트의 아이템 포인터를 가져온다.
	*	@pre	item을 입력받는다.
	*	@post	아이템 포인터 반환
	*	@return	일치하는 데이터를 찾으면 포인터 반환, 그렇지 않으면 NULL을 반환.
	*/
	T* GetPointer(T &item);

private:
	DoublyNodeType<T>* m_header; // 리스트의 맨 처음
	DoublyNodeType<T>* m_trailer; // 리스트의 맨 끝
...
DoublySortedLinkedList<T> :: DoublySortedLinkedList()
{
	...
	m_header->next = m_trailer; // 끝과 처음이 서로를 가리키게 초기화.
	m_header->prev = NULL; // 처음.

	m_trailer->next = NULL; // 끝.
	m_trailer->prev = m_header; // 끝과 처음이 서로를 가리키게 초기화.
...
int DoublySortedLinkedList<T>::Add(T item)
{
	...
	if(IsEmpty()) // 처음 삽입할 때
	{
		...
		m_trailer->prev = pItem; // 처음과 끝 사이에 삽입.
		m_nLength++;
		return 1;
	}
	else // 처음이 아닐 때
	{
		while(1)
		{
			if(item < itor.m_pCurPointer->data || itor.m_pCurPointer == m_trailer) // 맞는 자리를 찾는다.
			{
				DoublyNodeType<T> *pItem = new DoublyNodeType<T>;
				pItem->data = item;
...
void DoublySortedLinkedList<T>::Replace(T& item)
{
	...
	while(itor.m_pCurPointer != m_trailer)
	{
		...
		{
			item.SetRecordFromKB(); // 새로운 정보 입력받음
...
T* DoublySortedLinkedList<T>::GetPointer(T &item)
{
	...
	while(itor.m_pCurPointer != m_trailer)
	{
		...
		{
			item = itor.m_pCurPointer->data;
			return itor.Current(); // 일치하면 현재 아이템을 반환.
		}
		else
			itor.Next(); // 다음으로 이동.
	}

	return NULL; // 일치하지 않으면 NULL을 반환.
}
